...
`;
createFile(path.join(rootDir, 'src', 'main.script'), exampleScript);
createFile(path.join(rootDir, 'src', 'config.json'), configContent);
const contentTools = `Patch update until Beta 0.2.6`;
createFile(path.join(rootDir, 'tools', 'patcher.tls'), contentTools);
